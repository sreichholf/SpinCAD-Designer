 ...
 */ 
		...
		import com.holycityaudio.SpinCAD.SpinFXBlock;
 		import com.holycityaudio.SpinCAD.ControlPanel.ga_demo_vibratoControlPanel;
		
		public class ga_demo_vibratoCADBlock extends SpinCADBlock {

			private static final long serialVersionUID = 1L;
			private ga_demo_vibratoControlPanel cp = null;
			...
			public ga_demo_vibratoCADBlock(int x, int y) {
				super(x, y);
				setName("Vibrato");	
				...
				addControlInputPin(this, "Vibrato_Rate");
				addControlInputPin(this, "Vibrato_Width");
			...
			public void editBlock(){ 
				if(cp == null) {
					if(hasControlPanel == true) {
						cp = new ga_demo_vibratoControlPanel(this);
					}
				}
			}
			...
			sp = this.getPin("Vibrato_Rate").getPinConnection();
			int input1 = -1;
			if(sp != null) {
				input1 = sp.getRegister();
			}
			
			sp = this.getPin("Vibrato_Width").getPinConnection();
			int input2 = -1;
			if(sp != null) {
			...
			sfxb.FXallocDelayMem("vdel", 1000); 
			mono = sfxb.allocateReg();
			apout = sfxb.allocateReg();
			lp1 = sfxb.allocateReg();
			lp2 = sfxb.allocateReg();
			revout = sfxb.allocateReg();
			fvib = sfxb.allocateReg();
			lev = sfxb.allocateReg();
			strike = sfxb.allocateReg();
			widfil = sfxb.allocateReg();
			vib = sfxb.allocateReg();
			peak = sfxb.allocateReg();
			bypass = sfxb.allocateReg();
			sum = sfxb.allocateReg();
			...
			sfxb.loadSinLFO((int) SIN1,(int) 0, (int) 0);
			sfxb.readRegister(adcl, 0.5);
			sfxb.readRegister(adcr, 0.5);
			sfxb.FXwriteDelay("vdel", 0, 1);
			sfxb.writeRegister(mono, 0);
			sfxb.readRegister(sum, 0.5);
			...
			if(this.getPin("Vibrato_Rate").isConnected() == true) {
			sfxb.readRegister(input1, 1);
			sfxb.scaleOffset(0.3, 0.1);
			} else {
			sfxb.scaleOffset(0.0, 0.35);
			}
			
			sfxb.mulx(strike);
			sfxb.writeRegister(SIN1_RATE, 0);
			if(this.getPin("Vibrato_Width").isConnected() == true) {
			sfxb.readRegister(input2, 0.01);
			sfxb.readRegisterFilter(widfil, 0.001);
			sfxb.writeRegisterLowshelf(widfil, -1);
			} else {
			sfxb.scaleOffset(0.0, -0.01);
			}
			
			sfxb.writeRegister(SIN1_RANGE, 0);
			sfxb.FXchorusReadDelay(SIN1, SIN|REG|COMPC, "vdel+", 170);
			sfxb.FXchorusReadDelay(SIN1, SIN, "vdel+", 171);
			sfxb.writeRegister(vib, 0);
			sfxb.readRegister(mono, 1);
			sfxb.absa();
			sfxb.writeRegister(peak, 0.8);
			sfxb.readRegister(lev, -1);
			sfxb.skip(NEG, 2);
			sfxb.clear();
			sfxb.writeRegister(strike, 0);
			sfxb.clear();
			sfxb.readRegister(peak, 1);
			sfxb.maxx(lev, 0.999);
			sfxb.writeRegister(lev, 0);
			sfxb.readRegister(strike, 1);
			sfxb.writeRegister(strike, 0);
			if(this.getPin("Vibrato_Width").isConnected() == true) {
			...
			sfxb.readRegister(vib, 1);
			sfxb.readRegister(mono, -1);
			sfxb.mulx(bypass);
			sfxb.readRegister(mono, 1);
			sfxb.writeRegister(sum, 1);
