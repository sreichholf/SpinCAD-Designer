 ...
 * ga_demo_echo_repeatCADBlock.java
 ...
 */ 
		...
		public class ga_demo_echo_repeatCADBlock extends SpinCADBlock {
			...
			private ga_demo_echo_repeatControlPanel cp = null;
			...
			private double fbk = 0.25;

			public ga_demo_echo_repeatCADBlock(int x, int y) {
				super(x, y);
				setName("Repeat_echo");	
			...
			public void editBlock(){ 
				if(cp == null) {
					if(hasControlPanel == true) {
						cp = new ga_demo_echo_repeatControlPanel(this);
					}
				}
			}
			...
			sp = this.getPin("Delay_Level").getPinConnection();
			int input2 = -1;
			if(sp != null) {
			...
			if(this.getPin("Delay_Level").isConnected() == true) {
			sfxb.mulx(input2);
			sfxb.mulx(input2);
			} else {
			sfxb.scaleOffset(0.0, 0.25);
			}
