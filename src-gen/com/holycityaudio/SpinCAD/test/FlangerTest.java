...
public class FlangerTest {
	public FlangerTest(SpinCADFrame f) {
		System.out.println("Start of FlangerCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		FlangerCADBlock m = new FlangerCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("FlangerCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Center_Tap");
	p2.setConnection(o, p1);
}
{
...
f.getModel().sortAlignGen();
System.out.println("FlangerCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of FlangerCADBlock unit test" + "\n");
	}
}
