...
public class expanderTest {
	public expanderTest(SpinCADFrame f) {
		System.out.println("Start of expanderCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		expanderCADBlock m = new expanderCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("expanderCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Smooth");
	p2.setConnection(o, p1);
}
{
	SpinCADPin p1 = o.getPin("Input 2");
	SpinCADPin p2 = m.getPin("Raw");
	p2.setConnection(o, p1);
}
{
...
f.getModel().sortAlignGen();
System.out.println("expanderCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of expanderCADBlock unit test" + "\n");
	}
}
