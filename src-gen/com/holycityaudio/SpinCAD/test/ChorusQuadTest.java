...
public class ChorusQuadTest {
	public ChorusQuadTest(SpinCADFrame f) {
		System.out.println("Start of ChorusQuadCADBlock unit test..." + "\n");

		SpinCADPanel p = new SpinCADPanel(f);
		InputCADBlock i = new InputCADBlock(225, 10);
		OutputCADBlock o = new OutputCADBlock(225, 290);
		ChorusQuadCADBlock m = new ChorusQuadCADBlock(225, 210);
f.dropBlock(p, m);	

f.getModel().sortAlignGen();
System.out.println("ChorusQuadCADBlock test passed with no connections!");
f.getModel().newModel();
...
}
{
	...
	SpinCADPin p2 = m.getPin("Output_1");
	p2.setConnection(o, p1);
}
{
	SpinCADPin p1 = o.getPin("Input 2");
	SpinCADPin p2 = m.getPin("Output_2");
	p2.setConnection(o, p1);
}
{
	SpinCADPin p1 = o.getPin("Input 3");
	SpinCADPin p2 = m.getPin("Output_3");
	p2.setConnection(o, p1);
}
{
	SpinCADPin p1 = o.getPin("Input 4");
	SpinCADPin p2 = m.getPin("Output_4");
	p2.setConnection(o, p1);
}
{
...
f.getModel().sortAlignGen();
System.out.println("ChorusQuadCADBlock test passed with all control connections!");

f.getModel().newModel();	

// end of test code
	System.out.println("End of ChorusQuadCADBlock unit test" + "\n");
	}
}
